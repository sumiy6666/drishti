    ...
    const handleOtpSubmit = async (e) => {
        ...
        try {
            const res = await fetch((process.env.REACT_APP_API || 'http://localhost:5000') + '/api/auth/verify-otp', {
                ...
                body: JSON.stringify({ email, otp, type: type || 'register' })
            });
            ...
            if (data.token && data.user) {
                ...
                // Use window.location.href for a clean reload to the correct dashboard
                if (data.user.role === 'admin') window.location.href = '/admin';
                else if (data.user.role === 'employer') window.location.href = '/employer';
        ...
        <div className="min-h-screen bg-light flex items-center justify-center relative overflow-hidden px-4 py-20 pt-32">
            ...
            <div className="max-w-md w-full relative z-10">
                <div className="text-center mb-8">
                    ...
                    <h2 className="text-3xl font-bold text-dark mb-2">Verify Your Identity</h2>
                    <p className="text-body">We've sent a 6-digit code to <span className="text-secondary font-bold">{email}</span></p>
                </div>
                    ...
                    <form onSubmit={handleOtpSubmit} className="space-y-6 relative z-10 animate-fade-in-up">
                        ...
                        <div className="text-center space-y-4">
                            <button
                                type="button"
                                onClick={resendOtp}
                                className="text-primary font-bold hover:underline transition-colors block w-full"
                            >
                                Didn't receive code? Resend
                            </button>
                            <Link to="/login" className="text-body text-sm font-bold hover:text-dark transition-colors inline-block">
                                Back to Login
                            </Link>
                        </div>
                    </form>
